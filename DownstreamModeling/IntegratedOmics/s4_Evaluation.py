...
tgt_dir_lst = sort_nicely(glob.glob(dpath + 'Results/' + group + '/MultiOmics/IntegratedOmics/DirectIntegration/S3_RiskScore/TestData/*.csv'))
tgt_name_lst = [os.path.basename(tgt_dir)[:-4] for tgt_dir in tgt_dir_lst]

for tgt_name in tqdm(tgt_name_lst):
    tgt_pred_df = pd.read_csv(dpath + 'Results/' + group + '/MultiOmics/IntegratedOmics/DirectIntegration/S3_RiskScore/TestData/'+tgt_name+'.csv')
    ...
    res_eval.to_csv(dpath + 'Results/' + group + '/MultiOmics/IntegratedOmics/DirectIntegration/S4_Evaluation/'+tgt_name+'.csv', index=True)
